...
def _create_email_content(files_sent: List[str], files_skipped: List[str],
                          files_too_large: List[str], error_files: List[str], log_files: List[str]) -> str:
    ...
    lines.extend([
        ...
        "",
        "Generated by ArXiv AutoSumm"
    ])
    
    return "\n".join(lines)

def deliver(file_paths: List[str], config: DelivererConfig, subject: Optional[str]=None) -> DeliveryResult:
    files_sent = []
    files_skipped = []
    files_too_large = []
    error_files = []
    ...
    body = _create_email_content(files_sent, files_skipped, files_too_large, error_files, log_files)
